		...
		if (token[0] == '-') {
			...
			if (*(token +1) == 'v') {
				*arg = 1;
				...
				printf("ERROR. Opcion no sportada. Si necesitas ayuda ejecuta cp ?\n");
...
int ejecCp(char *prompt) {
	...
	if (validador == -1){				// linea vacia/token NULL pasamos
		...
		printf("Error. Cantidad de parametros insuficientes(%d). Si necesitas ayuda ejecuta cp ?\n", validador);
		return SH_USAGE;
	}
	else if(validador > 2) {
		printf("Error. Demasiados parametros (%d). Si necesitas ayuda ejecuta cp ?\n", validador);
		return SH_USAGE;
		...
		if (veriFuente == 0) {
			...
			if (fd_in == -1) {
				fprintf(stderr, "ERROR. No se pudo abrir el archivo %s. Si necesitas ayuda ejecuta cp ?\n", fuente);
				perror("open");
				return SH_SYSERR;
			}
			...
			if (veriDestino == 1) {
				...
				if (fd_out == -1) {
					fprintf(stderr, "ERROR. No se pudo crear/abrir el archivo %s. Si necesitas ayuda ejecuta cp ?\n", destino);
					perror("open");
					close(fd_in);
					return SH_SYSERR;
				}
				...
				fprintf(stderr, "ERROR. No se puede acceder al destino %s. Si necesitas ayuda ejecuta cp ?\n", destino);
				close(fd_in);
				return SH_SYSERR;
			}
			else {
				...
				if (stat(destino, &st) == -1) {
					fprintf(stderr, "ERROR. No se pudo inspeccionar el destino %s. Si necesitas ayuda ejecuta cp ?\n", destino);
					perror("stat");
				}
				else {
					fprintf(stderr, "ERROR. Destino %s no soportado: es %s. Si necesitas ayuda ejecuta cp ?\n",
			...
			fprintf(stderr, "ERROR. El archivo %s proporcionado como fuente es un directorio, no un archivo. Si necesitas ayuda ejecuta cp ?\n",fuente);
			return SH_USAGE;
		}
			...
			fprintf(stderr, "ERROR. El archivo %s proporcionado como fuente no existe. Si necesitas ayuda ejecuta cp ?\n",fuente);
			return SH_USAGE;
		}
			...
			fprintf(stderr, "ERROR. No se puede acceder al archivo %s proporcionado como fuente. Si necesitas ayuda ejecuta cp ?\n",fuente);
			return SH_SYSERR;
		}
		else {
			...
			struct stat st;
			if (stat(fuente, &st) == -1) {
				fprintf(stderr, "ERROR. No se pudo inspeccionar la fuente %s. Si necesitas ayuda ejecuta cp ?\n", fuente);
				perror("stat");
			} else {
				fprintf(stderr, "ERROR. Fuente %s no soportada: es %s. Si necesitas ayuda ejecuta cp ?\n", fuente, tipoArchivo(st.st_mode));
			}
			return SH_SYSERR;
		}
	}
