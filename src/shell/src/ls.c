...
int parsingLs (char *linea) {
	...
	while  ((token = strtok_r(NULL, " \t", &cadena)) != NULL) {	// 1er parametro NULL para que no se reinicie el proceso
		...
		if (token[0] == '?' && token[1] == '\0') {        // grep ?
	        int n = ayudaImpresion(9, 24);
	        if (n != 0) {
	        	return SH_SYSERR;
	        }
	        return -2;
...
int ejecLs (char *linea) {
	...
	if (modo != -1) {
		...
		if (directorio == NULL) {
			puts("Error al abrir en el archivo. Si necesitas ayuda escribe ls ?\n");
		...
		if (closedir(directorio) == -1) {
			puts ("Error al cerrar el archivo. Si necesitas ayuda escribe ls ?\n");
			return SH_SYSERR;
		}
	}
	return SH_OK;
